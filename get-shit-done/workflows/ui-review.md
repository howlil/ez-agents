...
**If `SUMMARY_FILES` empty:** Exit — "Phase {N} not executed. Run /ez:execute-phase {N} first."
...
- `/ez:verify-work {N}` — UAT testing
- `/ez:plan-phase {N+1}` — plan next phase
...
```bash
node "$HOME/.claude/ez-agents/bin/ez-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
