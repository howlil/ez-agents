...
Discovery only — never auto-calls `/ez:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/ez:validate-phase {N}` |

Phases needing validation: run `/ez:validate-phase {N}` for each flagged phase.
...
- /ez:complete-milestone {version} — proceed anyway (accept tech debt)
