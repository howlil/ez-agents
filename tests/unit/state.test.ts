...
`
    ...
    const outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'ez-test-outside-'));
...
`
    ...
    runEzTools('state update Status "In progress"', tmpDir);
    runEzTools('state update Status "Paused"', tmpDir);
