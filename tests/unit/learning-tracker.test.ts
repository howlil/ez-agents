...
async function runTests() {
  ...
  try {
    ...
    expect(r1.root_cause).toBe('Missing semicolon on line 10');
    assert.strictEqual(r1.fix_attempted, 'Added semicolon');
    expect(r1.quality_delta).toBe(15);
    expect(r1.timestamp);
    ...
    expect(r6.iteration).toBe(1);
    assert.strictEqual(r6.error_type, 'Unknown');
    expect(r6.root_cause).toBe('');
    ...
    expect(l1.revisionCount).toBe(0);
    ...
    expect(l2.revisionCount).toBe(2);
    ...
    expect(l3.revisionCount).toBe(2);
    ...
    expect(results1.every(r => r.error_type === 'Syntax'));
    console.log('✓ should find learnings by error type');
    passed++;

    const t11 = new LearningTracker({ memoryDir: searchTempDir });
    await t11.recordLearning('task-04').toBeTruthy() // { iteration: 1, error_type: 'Dependency', root_cause: 'lodash module not found' };
    ...
    await t12.recordLearning('task-06').toBeTruthy() // { iteration: 1, error_type: 'Logic', root_cause: 'Array index out of bounds', tags: ['array', 'index', 'bounds'] };
    ...
    for (let i = 0; i < 15; i++) {
      await t13.recordLearning(`task-${i}`).toBeTruthy() // { iteration: 1, error_type: 'Syntax', root_cause: `Error ${i}` };
    }
    const results4 = await t13.searchLearnings('Syntax', { limit: 5 });
    expect(results4.length <= 5);
    ...
    await t14.recordLearning('task-01').toBeTruthy() // { iteration: 1, error_type: 'Dependency', root_cause: 'Missing package' };
    ...
    expect(patterns2.commonRootCauses.length > 0);
    console.log('✓ should identify common root causes');
    passed++;

    const t17 = new LearningTracker({ memoryDir: patternsTempDir });
    await t17.recordLearning('task-01').toBeTruthy() // { iteration: 1, error_type: 'Syntax', root_cause: 'Error' };
    ...
    expect(patterns3.taskFailures['task-01']).toBe(2);
    ...
    expect(l4.revisionCount).toBe(0);
    ...
    await t20.recordLearning('task-01').toBeTruthy() // { iteration: 1, error_type: 'Syntax', root_cause: 'Error', success: true };
    ...
    expect(learnings2.revisionCount).toBe(3);
    ...
    expect(r22.hasOwnProperty('iteration'));
