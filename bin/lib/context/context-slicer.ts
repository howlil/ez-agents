...
export class ContextSlicer {
  ...
  async sliceContext(
    ...
    // Step 3: Enforce budget per tier
    const budgeted = this.enforceTierBudget(tiered);
    ...
    const stats = this.calculateStats(summarized, optimized);
  ...
  /**
   * Enforce token budget per tier with hard limits
   *
   ...
   *
   * Uses LLM abstractive summarization via CompressionStrategy.
   * Only summarizes sources with score below summarizationThreshold (default: 0.5).
   *
   * Engineering verdict: LLM abstractive via CompressionStrategy (not extractive or hybrid).
   * 10:1 ROI (49K net savings/phase), reuses existing code, preserves downstream agent clarity.
   *
  ...
  async summarizeLowRelevance(tiers: TieredSources, task: string): Promise<TieredSources> {
    ...
    for (const tier of ['hot', 'warm', 'cold'] as const) {
      for (const source of tiers[tier]) {
        if (source.score < this.config.summarizationThreshold) {
          try {
            // Use CompressionStrategy for abstractive summarization
            // Reuses existing infrastructure (no custom NLP pipeline)
            const options: CompressionOptions = {
              targetCompressionRatio: 0.67,  // 3:1 compression ratio
            ...
            // Fallback: Keep original content (no compression)
   ...
   */
  private calculateStats(tiers: TieredSources, optimized: ContextResult): SlicedContextResult['stats'] {
