...
import { extractFrontmatter, parseMustHavesBlock } from '../planning/index.js';
import { writeStateMd } from '../state/index.js';
import { defaultLogger as logger } from '../logger/index.js';

interface VerifySummaryResult {
  passed: boolean;
  checks: {
...
interface VerifyPlanStructureResult {
  valid: boolean;
  errors: string[];
  warnings: string[];
  task_count: number;
  tasks: Array<{ name: string; hasFiles: boolean; hasAction: boolean; hasVerify: boolean; hasDone: boolean }>;
  frontmatter_fields: string[];
}

interface VerifyPhaseCompletenessResult {
  complete: boolean;
  phase: string;
  plan_count: number;
...
interface VerifyReferencesResult {
  valid: boolean;
  found: number;
  missing: string[];
  total: number;
...
interface VerifyKeyLinksResult {
...
interface ValidateConsistencyResult {
  passed: boolean;
  errors: string[];
  warnings: string[];
...
interface ValidateHealthResult {
 ...
 * @param summaryPath - Path to summary file
 * @param checkFileCount - Number of files to spot-check
 * @param raw - Raw output flag
 */
export async function cmdVerifySummary(cwd: string, summaryPath: string, checkFileCount?: number, raw?: boolean): Promise<void> {
  if (!summaryPath) {
  ...
  if (!fs.existsSync(fullPath)) {
    const result: VerifySummaryResult = {
      passed: false,
      checks: {
  ...
  const checks: VerifySummaryResult['checks'] = {
    summary_exists: true,
...
export function cmdVerifyPhaseCompleteness(cwd: string, phase: string, raw?: boolean): void {
  ...
  try {
    ...
    logger.warn('Failed to read phase directory in cmdVerifyPhaseCompleteness', { phaseDir, error: (err as Error).message });
...
/**
 * Verify must_haves.artifacts exist and meet requirements
 * @param cwd - Working directory
...
export function cmdVerifyArtifacts(cwd: string, planFilePath: string, raw?: boolean): void {
  ...
  for (const artifact of artifacts) {
    ...
    const artPath = (artifact as Record<string, unknown>).path as string;
    if (!artPath) continue;
  ...
  for (const link of keyLinks) {
    ...
    if (!sourceContent) {
      ...
      try {
        ...
        logger.warn('Invalid regex while verifying key links', { pattern: linkObj.pattern, error: (err as Error).message });
...
/**
 * Validate consistency between ROADMAP.md and phase directories
 * @param cwd - Working directory
 * @param raw - Raw output flag
 */
export function cmdValidateConsistency(cwd: string, raw?: boolean): void {
  ...
  try {
    ...
    logger.warn('Failed to validate plan numbering while validating consistency', { phasesDir, error: (err as Error).message });
  }
  ...
  try {
    ...
    logger.warn('Failed to validate plan frontmatter while validating consistency', { phasesDir, error: (err as Error).message });
  }
...
export function cmdValidateHealth(cwd: string, options: HealthOptions = {}, raw?: boolean): void {
  ...
  if (!fs.existsSync(statePath)) {
    ...
    try {
      ...
      logger.warn('Failed to read phase directories while validating STATE references', { phasesDir, error: (err as Error).message });
    }
  ...
  if (!fs.existsSync(configPath)) {
    ...
    try {
      ...
      logger.warn('Failed to parse config.json in cmdValidateHealth', { configPath, error: (err as Error).message });
      addIssue('error', 'E005', `config.json: JSON parse error - ${(err as Error).message}`, 'Run /ez:health --repair to reset to defaults', true);
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (configParsed.workflow && typeof configParsed.workflow === 'object' && (configParsed.workflow as Record<string, unknown>).nyquist_validation === undefined) {
      ...
      logger.warn('Failed to parse config for nyquist key check', { configPath, error: (err as Error).message });
    }
  }
  ...
  try {
    ...
    logger.warn('Failed to check phase directory naming in cmdValidateHealth', { phasesDir, error: (err as Error).message });
  }
  ...
  try {
    ...
    logger.warn('Failed to inspect orphaned plans in health validation', { phasesDir, error: (err as Error).message });
  }
  ...
  try {
    ...
    logger.warn('Failed to inspect validation architecture consistency in health validation', { phasesDir, error: (err as Error).message });
  }
  ...
  if (fs.existsSync(roadmapPath)) {
    ...
    try {
      ...
      logger.warn('Failed to run roadmap/disk consistency checks in health validation', { roadmapPath, phasesDir, error: (err as Error).message });
    }
  ...
  if (options.repair && repairs.length > 0) {
    for (const repair of repairs) {
      try {
          ...
          case 'addNyquistKey': {
            if (fs.existsSync(configPath)) {
              try {
                ...
                if (typeof configParsed.workflow !== 'object') configParsed.workflow = {};
                const workflow = configParsed.workflow as Record<string, unknown>;
                if (workflow.nyquist_validation === undefined) {
                  workflow.nyquist_validation = true;
                  fs.writeFileSync(configPath, JSON.stringify(configParsed, null, 2), 'utf-8');
                }
                repairActions.push({ action: repair, success: true, path: 'config.json' });
              } catch (err) {
                logger.error('Failed to repair nyquist key', { error: (err as Error).message });
        ...
        logger.error('Failed to perform repair action', { action: repair, error: (err as Error).message });
        repairActions.push({ action: repair, success: false, error: (err as Error).message });
      }
    }
  }
