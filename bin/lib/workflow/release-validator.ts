...
interface ReleaseValidationResult {
  ...
  warnings: string[];
  score: number;
  readinessStatus: 'READY' | 'CONDITIONAL' | 'NOT READY';
  securityGates: SecurityGatesResult;
 ...
 * @returns Security gates result
 */
export function runSecurityGates(cwd = process.cwd()): SecurityGatesResult {
...
const MVP_CHECKLIST: ChecklistItem[] = [
...
const MEDIUM_EXTRA: ChecklistItem[] = [
...
const ENTERPRISE_EXTRA: ChecklistItem[] = [
 ...
 * @returns Rollback validation result
 */
export function validateRollbackContent(cwd: string): RollbackValidationResult {
...
// ─────────────────────────────────────────────
// Manual Checklist State Persistence
// ─────────────────────────────────────────────

/**
 * Load persisted manual checklist state
 * @param cwd - Working directory
 * @returns Checklist state object
 */
export function loadChecklistState(cwd: string): Record<string, ManualCheckState> {
 ...
 * @param cwd - Working directory (default: process.cwd())
 */
export function markManualItemComplete(itemId: string, approver: string, cwd = process.cwd()): void {
 ...
 * @param tier - Tier name (mvp, medium, enterprise)
 * @returns Checklist items
 */
export function getChecklist(tier: string): ChecklistItem[] {
 ...
 * @param cwd - Working directory (default: process.cwd())
 * @param context - Additional context (coverage, bddResults, etc.)
 * @returns Checklist result
 */
export function runChecklist(tier: string, cwd = process.cwd(), context: ChecklistContext = {}): ChecklistResult {
  ...
  for (const item of items) {
    ...
    if (!item.auto) {
      // Check persisted state for manual items
      const state = loadChecklistState(cwd);
...
function runChecklistItem(item: ChecklistItem, cwd: string, context: ChecklistContext): ChecklistItem {
  try {
    switch (item.id) {
      case 'npm_audit':
      case 'no_secrets':
      ...
      case 'bdd_must': {
        ...
        if (moscowTagged !== undefined && totalScenarios !== undefined && totalScenarios > 0) {
    ...
    return { ...item, status: 'error', detail: (err as Error).message };
  }
}
 ...
 */
export function validateRelease(tier: string, version: string, cwd = process.cwd(), context: ChecklistContext = {}): ReleaseValidationResult {
 ...
 */
export function formatValidation(result: ReleaseValidationResult): string {
  const lines: string[] = [];
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    if (cmd === 'security-gates') {
      ...
      const tier = args[1];
      if (!tier) { console.error('Usage: release-validator.ts checklist <tier>'); process.exit(1); }
      ...
      if (!tier) { console.error('Usage: release-validator.ts validate <tier> [version]'); process.exit(1); }
      ...
      if (!itemId || !approver) {
        console.error('Usage: release-validator.ts checklist-mark <item-id> <approver>');
        process.exit(1);
      }
