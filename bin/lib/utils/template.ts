...
import { reconstructFrontmatter } from '../planning/index.js';
...
/**
 * Select appropriate template based on plan content
...
/**
 * Fill template with provided options
 * @param cwd - Working directory
 * @param templateType - Template type: summary, plan, or verification
 * @param options - Template fill options
 * @param raw - Raw output flag
 */
export function cmdTemplateFill(cwd: string, templateType: string, options: TemplateFillOptions, raw?: boolean): void {
  ...
  const phaseSlug = phaseInfo.phase_slug || generateSlugInternal(phaseName) || 'unnamed';
  ...
  switch (templateType) {
    ...
    case 'plan': {
      ...
      const wave = parseInt(options.wave as string) || 1;
      frontmatter = {
        phase: phaseId,
        plan: planNum,
        type: planType,
